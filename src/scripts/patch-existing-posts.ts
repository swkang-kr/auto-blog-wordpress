...
const NICHE_DISCLAIMERS: Record<string, string> = {
  'Korean Finance': '<div class="ab-disclaimer-finance" style="margin:0 0 24px 0; padding:16px 20px; background:#fffbeb; border:1px solid #fde68a; border-radius:8px; font-size:13px; color:#666; line-height:1.6;"><strong>Financial Disclaimer:</strong> This content is for informational purposes only and does not constitute financial advice. Investment decisions carry risk. Past performance is not indicative of future results. Consult a qualified financial advisor before making investment decisions.</div>',
...
async function main() {
  ...
  for (const post of allPosts) {
    ...
    const catName = post.categories.map((c) => catMap.get(c) || '').find((n) =>
      ['Korean Tech', 'Korean Finance', 'K-Beauty', 'K-Entertainment', 'Korea Travel'].includes(n)
    ) || '';
