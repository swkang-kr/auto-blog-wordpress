 ...
 *  3. Board 생성/확인 (K-Beauty & Skincare)
...
async function main() {
  ...
  // ── Step 3: Ensure K-Beauty board exists ──
  const targetBoard = 'K-Beauty & Skincare';
  ...
  if (!boardId) {
    ...
    try {
      const pinBody = {
        ...
        alt_text: `${wpPost.title} — K-Beauty guide`.substring(0, 500),
      };
