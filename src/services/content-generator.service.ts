...
const NICHE_SIGNATURE_SECTIONS: Record<string, Record<string, string[]>> = {
  ...
  '업종분석': {
    default: ['Sector Watch', 'Industry Deep Dive', 'Sector Insight', 'Sector Verdict'],
    'analysis': ['Sector Data Analysis', 'Industry Chart Analysis'],
    'deep-dive': ['Industry Deep Dive', 'Sector Insight'],
    'news-explainer': ['Sector Impact', 'Industry Takeaway'],
    'how-to': ['Pro Tips', 'Step-by-Step Guide'],
    'x-vs-y': ['Sector Comparison Verdict', 'Industry Head-to-Head'],
    'best-x-for-y': ['Top Sector Picks', 'Industry Analysis'],
    'listicle': ["Sector Analyst's Picks", 'Industry Watch'],
    'case-study': ['Sector Performance Analysis', 'Industry Lessons'],
  },
  '테마분석': {
    default: ['Theme Insight', 'Theme Data Analysis', 'Theme Takeaway', 'Theme Watch'],
    'analysis': ['Theme Chart Analysis', 'Data Deep Dive'],
    'deep-dive': ['Theme Deep Dive', 'Theme Insight'],
    'news-explainer': ['Theme Impact', 'Policy Takeaway'],
    'how-to': ['Pro Tips', 'Step-by-Step Guide'],
    'x-vs-y': ['Theme Comparison Verdict', 'Head-to-Head Analysis'],
    'best-x-for-y': ['Top Theme Picks', 'Theme Analysis'],
    'listicle': ["Theme Analyst's Picks", 'Theme Watch'],
    'case-study': ['Theme Performance Analysis', 'Theme Lessons Learned'],
  },
  '종목분석': {
    default: ['오늘의 매수후보 총평', '기술적 분석 종합', '수급 분석 요약', '매수 타이밍 정리'],
    'analysis': ['오늘의 매수후보 기술적 분석 결과', '매수 타이밍 종합 판단'],
    'deep-dive': ['기술적 지표 심층 분석', '수급 + 차트 종합 분석'],
    'how-to': ['매수후보 선별 실전 가이드', '투자 참고 팁'],
    'case-study': ['매수후보 분석 사례', '교훈 및 정리'],
    'x-vs-y': ['종목 비교 종합 판단', '매수후보 비교 분석'],
    'news-explainer': ['공시·이슈 영향 분석', '매수 타이밍 시사점'],
    'best-x-for-y': ['오늘의 매수후보 순위', '종목별 종합 점수'],
    'listicle': ['오늘의 매수후보 리스트', '기술적 분석 요약'],
  },
  // Legacy keys (dead — old category names no longer used)
  '_legacy_kbeauty': {
    default: ['Market Insight', 'Data Analysis', 'Investment Takeaway', 'Sector Watch'],
    'analysis': ['Chart Analysis', 'Data Deep Dive', 'Market Insight'],
    'deep-dive': ['Fundamental Analysis', 'Market Insight'],
...
- **종목분석·시장분석 product-review**: 투자 리스크 등급(저위험/중위험/고위험), 적합 투자자 유형(초보/중급/고급), 최소 투자금액 기준, 유동성(일평균 거래량), 매매 가능 플랫폼(키움, 미래에셋, 삼성증권 MTS 등) 포함.
...
- 한국주식 지표: DART 공시 분석, 주가 차트 데이터, PER/PBR 변화 추이, 섹터 비교
...
## 니치별 톤
- 시장분석/업종분석/테마분석: 전문 주식 분석가 — DART 공시, 기술적 분석, 기업 재무제표를 기반으로 데이터 중심 분석. 구체적 수치(PER, PBR, ROE, 시가총액)와 차트 데이터를 반드시 포함. 투자 추천이 아닌 분석 제공. "~할 수 있다", "~로 보인다" 등 헤지 언어 사용.
- 종목분석: 현장 트레이더 & 기술적 분석 전문가 — 오늘의 매수후보 종목을 RSI·MACD·볼린저밴드·수급(외국인·기관)·차트 패턴 기반으로 분석. 구체적인 매수 타이밍·손절선·목표가 제시. 투자 추천이 아닌 분석 정보 제공임을 명시.
...
- Example (시장분석): <h3 id="삼성전자-주가-전망-2026" style="...">삼성전자 주가 2026년 전망은?</h3>
- Example (종목분석): <h3 id="MACD-골든크로스-매수-타이밍" style="...">MACD 골든크로스 발생 시 매수 타이밍은?</h3>
...
- SOURCE_KEY values (use exactly these keys):
  ...
  * 종목분석 platforms: quantinvest, backtrader, zipline, quant-kor, naver-finance-api
...
- When referencing product prices or sales data: use "as of [month] ${new Date().getFullYear()}" or "according to recent listings" — avoid exact prices unless explicitly provided in the prompt, as stock/financial data changes frequently across platforms
...
- Good (시장분석): "서울 여의도 증권가 전경과 KOSPI 전광판, 주식 투자자들이 차트를 분석하는 모습"
- Good (종목분석): "오늘의 매수후보 종목 캔들스틱 차트에 RSI MACD 지표가 오버레이된 트레이딩 화면"
...
- **시장분석/업종분석**: 섹터별 수익률 막대 차트, PER/PBR 밸류에이션 비교 차트, 외국인/기관 순매수 추이, 업종 강도 히트맵
- **종목분석**: 매수후보 종목별 RSI·MACD 시그널 강도 비교 막대 차트, 수급(외국인·기관 순매수) 비교, 기술적 지표 종합 점수 시각화

Keep SVG charts simple: max 5 bars/items, clear labels, brand colors (#0066FF, #00CC66, #FF6B35).

### Key Metrics Highlight (한국 주식시장 분석)
Display key numbers prominently — 시장분석/업종분석/테마분석: PER, 시가총액, 52주 고/저, 외국인 보유비율; 종목분석: RSI 수치, 현재가, 매수 타이밍 구간, 손절선, 목표가:
...
### Infographic-Style Data Box (한국 주식시장 분석 — data-heavy content)
...
export class ContentGeneratorService {
   ...
   *
   * 시장분석/업종분석: Daniel Park (primary) → Jiwon Lee (secondary: macro)
   * 테마분석/종목분석: Daniel Park (primary) → Sungho Choi (secondary: systems/quant)
   */
  selectAuthorPersona(category: string, contentType: string, postCount: number, keyword?: string): AuthorProfile {
    ...
    // 시장분석/업종분석: macro/interest-rate/currency → Jiwon Lee (secondary)
    if (['시장분석', '업종분석'].includes(category) && keyword) {
    ...
    // 종목분석: system architecture/quant infrastructure → Sungho Choi (secondary)
    if (category === '종목분석' && keyword) {
      const kw = keyword.toLowerCase();
      if (kw.includes('architecture') || kw.includes('websocket') || kw.includes('dashboard') ||
          ...
          kw.includes('quant') || kw.includes('algorithm') || kw.includes('backtest')) {
...
These are sibling posts in your topic cluster. You MUST include a natural contextual link to each one.`;
    ...
    if (existingPosts && existingPosts.length > 0) {
      ...
      // 한국 주식시장 niches natural bridges: 종목분석, 퀀트 전략, 시장 분석 공통 주제
...
If you need to reference a topic with no matching URL below, just write the text WITHOUT a link.\n${postList}`;
    ...
    const nicheDirectives: Record<string, string> = {
      '시장분석': `NICHE VOICE: Write as an experienced Korean stock market analyst. Use financial terminology correctly: PER (not P/E in Korean context), PBR (not P/B), ROE, EPS, BPS. Reference KOSPI/KOSDAQ indices, KRX market data, and DART filings. Include specific stock codes (e.g., Samsung Electronics 005930.KS). Always note whether analysis is technical, fundamental, or both.
...
${analysis.contentType === 'case-study' ? 'CASE STUDY STRUCTURE: Focus on ONE stock, sector, or market event. Structure: Context & Background → Catalyst/Trigger → Market Reaction (with chart data) → Fundamental Impact → Lessons for Investors. Include specific dates, prices, and volume data.' : ''}`,
      ...
      '종목분석': `NICHE VOICE: Hands-on Korean stock trader & technical analyst. Focus on TODAY's buy candidates — actionable, timely analysis of specific stocks based on RSI·MACD·Bollinger Band signals, foreign/institutional net buying, and chart patterns.

STRUCTURE: 오늘의 매수후보 요약 → 종목별 기술적 분석 (RSI·MACD·BB) → 수급 분석 (외국인·기관 순매수) → 매수 타이밍·손절선·목표가 → 투자 유의사항.

DISCLAIMER: Include trading disclaimer. "이 글은 투자 정보 제공 목적이며 특정 종목 매수·매도를 권유하지 않습니다. 투자 결정은 본인 판단과 책임 하에 이루어져야 합니다."

TECHNICAL ACCURACY: RSI(14) daily — oversold<30, overbought>70. MACD(12,26,9) — golden/dead cross. BB(20,2) — band support/resistance. Always confirm with volume surge.

RISK MGMT: Always present 손절선 (stop-loss) and 목표가 (target price) for each candidate. Risk/reward ratio ≥ 1:2.
...
Korean institutional sources preferred: bok, krx, dart, kosis, naver-finance, investing-kr, sedaily, hankyung.
...
${['시장분석', '업종분석'].includes(niche.category) ? '- Cite data sources: 네이버 금융 시세/재무, KRX 시장 데이터, DART 공시 시스템, 한국은행 경제통계(ECOS), 금융감독원(FSS) 보고서' : niche.category === '종목분석' ? '- Cite data sources: Python 라이브러리(FinanceDataReader, pykrx, backtrader), 퀀트 연구 자료, 증권사 백테스트 보고서, KOSPI/KOSDAQ 히스토리 데이터, DART 공시' : '- Cite specific Korean data sources: BOK (Bank of Korea), KOSIS (Korean Statistical Information Service), DART (disclosure system), or industry reports'}
...
${(['product-review', 'best-x-for-y', 'x-vs-y', 'listicle', 'how-to'].includes(analysis.contentType) && ['시장분석', '업종분석', '테마분석', '종목분석'].includes(niche.category)) ? `
...
Respond with pure JSON only.`;
    ...
    if (this.siteOwner) {
      ...
      const NICHE_BYLINE_BIO: Record<string, string> = {
        '시장분석': '한국 주식시장 분석 전문 | KOSPI·KOSDAQ 기술적 분석, 시장 흐름, 매크로 데이터 기반 투자 인사이트 제공.',
        '업종분석': '한국 주식시장 업종 분석 전문 | 반도체·2차전지·바이오 등 섹터 분석, 실적 데이터 기반 투자 인사이트 제공.',
        '테마분석': '한국 테마주 분석 전문 | 정책 수혜 테마, AI·로봇·친환경 테마주 동향 및 투자 인사이트 제공.',
        '종목분석': '알고리즘 트레이딩 전문 | 퀀트 전략 설계·백테스트·Python 구현 가이드. 한국 주식시장 자동매매 시스템 구축 노하우 공유.',
      };
