...
` : ''}한국어로 "${analysis.selectedKeyword}" 주제의 ${analysis.contentType} 블로그 포스트를 작성하세요. ${niche.name} 니치입니다. 최소 ${getWordCountTargets(analysis.contentType, analysis.searchIntent).target}단어 이상 작성하세요. 각 섹션을 상세히 — 한국 시장 데이터와 전문가 인사이트를 포함하여 깊이 있게 작성하세요. 중간에 멈추지 마세요. 모든 텍스트는 한국어로 작성하세요. slug도 한국어로 작성하세요 (예: "휴니드-외국인-순매수-분석-2026"). 기술 용어(RSI, MACD, PER 등)는 영문 그대로 사용 가능.
