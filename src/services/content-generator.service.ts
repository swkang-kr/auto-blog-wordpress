...
LABEL ACCURACY (prevent common AI errors): IVE → Starship Entertainment (NOT HYBE). aespa, SHINee, EXO, NCT units → SM Entertainment (NOT HYBE). BABYMONSTER → YG Entertainment (NOT HYBE). HYBE labels include: Big Hit Music (BTS, TXT), PLEDIS Entertainment (SEVENTEEN, TWS), SOURCE MUSIC (LE SSERAFIM), Belift Lab (ENHYPEN), ADOR (NewJeans — but members left ADOR in 2025), KOZ Entertainment (Zico). Always verify a group's label before stating it — mislabeling is a high-visibility accuracy error.
