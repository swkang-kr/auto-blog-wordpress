...
const NICHE_SIGNATURE_SECTIONS: Record<string, Record<string, string[]>> = {
  ...
  '종목분석': {
    default: ['Strategy Insight', 'Technical Deep Dive', 'Stock Analysis', 'Risk Analysis'],
    'analysis': ['Technical Analysis Results', 'Strategy Insight'],
    'deep-dive': ['Technical Deep Dive', 'Fundamental Analysis'],
    'how-to': ['Trading Guide', 'Pro Tips'],
    'case-study': ['Performance Analysis', 'Lessons Learned'],
    'x-vs-y': ['Head-to-Head Verdict', 'Stock Comparison'],
    'news-explainer': ['Market Impact Watch', 'Strategy Insight'],
    'best-x-for-y': ['Stock Picks Comparison', 'Strategy Insight'],
    'listicle': ["Analyst's Picks", 'Stock Comparison'],
  },
...
- 종목분석: 퀀트/알고리즘 트레이딩 전문가 — 전략 이론 + Python 구현 코드를 함께 제공. 백테스트 결과(수익률, 승률, MDD, 샤프 비율)를 반드시 포함. 실전과 백테스트의 차이를 항상 명시.
...
- Include Korean visual elements where appropriate: 시장분석/업종분석/테마분석 content → 서울 여의도 증권가 빌딩, 주식 차트 모니터, 한국거래소(KRX) 전광판, 투자자 분석 화면, 캔들스틱 차트 클로즈업; 종목분석 content → Python 코드 화면, 백테스트 그래프, 알고리즘 트레이딩 대시보드, 서버 랙과 데이터 시각화
...
- Good (종목분석): "Python 알고리즘 트레이딩 백테스트 결과 차트가 표시된 듀얼 모니터 트레이딩 데스크"
...
- **종목분석**: 전략별 백테스트 수익률 비교 막대 차트, 월별 승률 차트, MDD(최대 낙폭) 비교 시각화

Keep SVG charts simple: max 5 bars/items, clear labels, brand colors (#0066FF, #00CC66, #FF6B35).

### Key Metrics Highlight (한국 주식시장 분석)
Display key numbers prominently — 시장분석/업종분석/테마분석: PER, 시가총액, 52주 고/저, 외국인 보유비율; 종목분석: 백테스트 수익률, 승률, MDD, 샤프 비율:
