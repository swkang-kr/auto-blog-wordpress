...
K-POP FANDOM NAMES (use official names — signals fan content authenticity): BTS → ARMY, BLACKPINK → BLINK, TWICE → ONCE, SEVENTEEN → CARAT, Stray Kids → STAY, ATEEZ → ATINY, ENHYPEN → ENGENE, TXT (Tomorrow X Together) → MOA, aespa → MY, IVE → DIVE, LE SSERAFIM → FEARNOT, BABYMONSTER → MONSTER, NewJeans/NJZ → Bunnies, ILLIT → LLIT, NCT (all units) → NCTzen. Always use the official fandom name at least once per article when writing about a specific group. Never write "BTS fans" without also noting "ARMY" — it signals unfamiliarity with fan culture.
